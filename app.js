...
// Lookback + Confetti + Sound + Mood + Memories + Reactions
// Sandbox (no password)
// ==========================
...
const moodBadBtn = document.getElementById("moodBad");
...
// Put ambience.mp3 where your repo can find it.
// Default: file in repo ROOT: /ambience.mp3
const AMBIENCE_SRC = "./ambience.mp3";

partyBtn.addEventListener("click", () => {
  if (!ambienceAudio) {
    ambienceAudio = new Audio(AMBIENCE_SRC);
    ambienceAudio.loop = true;
    ambienceAudio.volume = 0.95; // louder
  }
  if (!soundOn()) {
    ambienceAudio.pause();
    return;
...
function updateReadOnlyUI() {
  ...
  historyStatusEl.textContent = isToday
    ? ""
    : "🔒 Viewing the past. Posting + mood check-in works only for today.";

  postBtn.disabled = !isToday;
  moodGoodBtn.disabled = !isToday;
  moodOkBtn.disabled = !isToday;
  moodBadBtn.disabled = !isToday;
}

function setSelectedDate(iso) {
  SELECTED_DATE = iso;
  historyDateEl.value = iso;
...
function updateMoodBoard(checkins) {
  ...
  if (checkins.length === 0) {
    moodBoardEl.innerHTML = `<div class="muted">No check-ins.</div>`;
    return;
  }
  ...
  moodBoardEl.innerHTML = checkins
    ...
    .map(c => `
      <div style="padding:10px 12px; border:1px solid rgba(20,20,40,0.10); border-radius:14px; margin:8px 0; background:rgba(255,255,255,0.92);">
...
async function toggleReaction(memoryIdRaw, emoji) {
  ...
  if (sel.error) { alert("Reaction select error: " + sel.error.message); return; }

  if (sel.data && sel.data.length) {
    const del = await supa.from("reactions").delete().eq("id", sel.data[0].id);
    if (del.error) { alert("Reaction delete error: " + del.error.message); return; }
  } else {
    ...
    if (ins.error) { alert("Reaction insert error: " + ins.error.message); return; }
  }
...
async function postMemory() {
  ...
  const ins = await supa.from("memories").insert([{ room_code: room, name, moment }]);
  if (ins.error) {
    statusEl.textContent = "Error: " + ins.error.message;
    return;
  }
...
postBtn.addEventListener("click", postMemory);

// ---- Mood check-in (today only)
function clearMoodSelection() {
  moodGoodBtn.classList.remove("moodSelected");
  moodOkBtn.classList.remove("moodSelected");
  moodBadBtn.classList.remove("moodSelected");
}
...
async function setMood(mood) {
  ...
  if (up.error) {
    moodStatusEl.textContent = "Error: " + up.error.message;
    return;
  }

  clearMoodSelection();
  if (mood === "good") moodGoodBtn.classList.add("moodSelected");
  if (mood === "ok") moodOkBtn.classList.add("moodSelected");
  if (mood === "bad") moodBadBtn.classList.add("moodSelected");

  moodStatusEl.textContent = "Checked in ✅";
  loadForSelectedDate();
}

moodGoodBtn.addEventListener("click", () => setMood("good"));
moodOkBtn.addEventListener("click", () => setMood("ok"));
moodBadBtn.addEventListener("click", () => setMood("bad"));
...
async function loadForSelectedDate() {
  try {
    ...
    kpiMemoriesEl.textContent = String(memories.length);
    kpiCheckinsEl.textContent = String(checkins.length);
    kpiMoodEl.textContent = vibe;
    kpiReactsEl.textContent = String(reactions.length);
    ...
    recapEl.textContent = `${SELECTED_DATE}: ${memories.length} memories • ${checkins.length} check-ins • ${reactions.length} reactions`;

    updateMoodBoard(checkins);

    // Render feed (no blinking if nothing changed)
      ...
      listEl.innerHTML = memories.map(m => {
        ...
        return `
          <div class="memoryCard">
            ...
            <span class="muted"> — ${escapeHtml(fmtLocal(m.created_at))}</span>
            <div style="margin-top:6px;">${escapeHtml(m.moment)}</div>

            <div class="memoryActions">
              <button class="btn reactBtn" data-mid="${m.id}" data-emo="❤️" type="button">❤️ ${rx["❤️"]}</button>
              <button class="btn reactBtn" data-mid="${m.id}" data-emo="😂" type="button">😂 ${rx["😂"]}</button>
              <button class="btn reactBtn" data-mid="${m.id}" data-emo="⭐" type="button">⭐ ${rx["⭐"]}</button>
            </div>
          </div>
        `;
